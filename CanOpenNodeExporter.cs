...
");
    ...
    public static class StringUnescape
    {
        ...
        {
            if (args.Length != 2) {
                System.Console.WriteLine("mono CanOpenNodeExporter.exe EDS_FILE OUTPUT_FOLDER");
                Environment.Exit(1);
            }

            CanOpenNodeExporter exporter = new CanOpenNodeExporter();
            EDSsharp eds = new EDSsharp();
            eds.loadfile(args[0]);
            exporter.export(args[1], "4.0", eds);
        }
    }
